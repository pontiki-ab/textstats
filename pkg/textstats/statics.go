...
func WordsWithAtLeastNSyllables(text string, n int) int {
	res, _ := Analyse(strings.NewReader(text))
	return res.WordsWithAtLeastNSyllables(n)
}
...
func PercentageWordsWithAtLeastNSyllables(text string, n int) float64 {
	res, _ := Analyse(strings.NewReader(text))
	return res.PercentageWordsWithAtLeastNSyllables(n)
}

// WordCount returns the number of words in a given string
func WordCount(text string) int {
	res, _ := Analyse(strings.NewReader(text))
	return res.CountTotalWords
}
...
// GetSyllableCount returns the number of syllables in a given string
func GetSyllableCount(text string) int {
	res, _ := Analyse(strings.NewReader(text))
